...
@csrf_exempt
def checkAllHashes(request):
    response = {

    }
    received_json = (request.GET.dict())
    isFound = True
    for block in blockchain.chain:
        block_hash = blockchain.hash(block)
        if(block_hash == received_json['item']):
            response['message'] = "SUCCESS! Block found! Item Valid"
            return JsonResponse(response)
    response['message'] = "PRODUCT INVALID!"
    return JsonResponse(response)
